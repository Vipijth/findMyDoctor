...
<tr>
<td>";
  $n = mysqli_real_escape_string($link, $_REQUEST['user']);
  echo '$n';
$sqt="delete from chat where  user like '$n' ";
...
$sql="SELECT distinct(user) FROM chat";
echo "<br><br>";
echo "";
     $retval = mysql_query( $sql, $conn );
...
<tr>
<td border=1px><center><form action='' method='post'>

<FONT COLOR='WHITE' FACE='KALINGA'>Message From:&nbsp;&nbsp;&nbsp;<input type='text' name='user' readonly style='border-radius:0px;background:#526;box-shadow:2px 2px 2px 2px black; color:white; width:225;text-align:center;' value='{$row['user']}'>


&nbsp;&nbsp;&nbsp;&nbsp;

<input type='submit' name='dl' class='lk' style='border-radius:0px;box-shadow:2px 2px 2px 2px black; color:white; width:225;text-align:center;' value='Delete'>
</form><form action='chat1.php#s' method='post'><br>
<input type='hidden' name='usr' value='{$row['user']}'>
<input type='submit' name='vms' class='lk' style='border-radius:0px;box-shadow:2px 2px 2px 2px black; color:white; width:225;text-align:center;' value='View Message'><hr/>
